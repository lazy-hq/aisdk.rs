...
AISDK provides first-class support for Zai with fully typed model APIs.
...
Enable the Zai provider feature:

```bash
cargo add aisdk --features zai
```

This installs AISDK with the Zai provider enabled. Once you have enabled the Zai provider, you can use all aisdk <Link href="/docs#core-features">features</Link> with it.

## Create a Provider Instance

To create a provider instance, call `Zai::model_name()`, where **model_name** is the Zai model you want to use.
Model names are exposed as snake-case methods.

```rust
use aisdk::providers::Zai;
...
* Zai's default base URL (https://api.z.ai/api/paas/v4)
...
You can customize provider configuration using `Zai::builder()`

### API Key

```rust
let zai = Zai::<Glm45>::builder()
...
let zai = Zai::model_name("glm-4.5");
```
...
let zai = Zai::<DynamicModel>::builder()
	.model_name("glm-4.5")
	.api_key("your-api-key")
	.base_url("https://api.z.ai/api/paas/v4")
	.provider_name("Zai")
...
For compile-time safety, use the typed methods like `Zai::glm_4_5()`.
</Callout>
