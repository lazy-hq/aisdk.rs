...
let google = Google::gemini_1_5_flash();
```
...
```rust
let google = Google::<Gemini15Flash>::builder()
...
let google = Google::model_name("gemini-1.5-flash");
```
...
For compile-time safety, use the typed methods like `Google::gemini_1_5_flash()`.
</Callout>
