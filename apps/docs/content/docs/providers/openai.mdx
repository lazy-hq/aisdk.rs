...
let openai = OpenAI::codex_mini_latest();
```
...
```rust
let openai = OpenAI::<CodexMiniLatest>::builder()
...
let openai = OpenAI::model_name("codex-mini-latest");
```
...
For compile-time safety, use the typed methods like `OpenAI::codex_mini_latest()`.
</Callout>
