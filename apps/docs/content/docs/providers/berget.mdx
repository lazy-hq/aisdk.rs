...
AISDK provides first-class support for Berget with fully typed model APIs.
...
This installs AISDK with the Berget provider enabled. Once you have enabled the Berget provider, you can use all aisdk <Link href="/docs#core-features">features</Link> with it.

## Create a Provider Instance

To create a provider instance, call `Berget::model_name()`, where **model_name** is the Berget model you want to use.
...
For compile-time safety, use the typed methods like `Berget::baai_bge_reranker_v2_m3()`.
</Callout>
