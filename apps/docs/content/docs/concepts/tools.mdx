...
Tool usage is capability-guarded at compile time when you pick models via typed model methods (for example `OpenAI::gpt_5()`). See [Capability Safety](/docs/concepts/generating-text#the-capability-system) in the Generating Text docs.
...
To make a tool available to a model, pass it to the `.with_tool()` method in the [`LanguageModelRequest`](/docs/concepts/generating-text) builder.
...
To register manually-defined tools with the AI model, you need to add them to [`LanguageModelRequest`](/docs/concepts/generating-text) builder using the `with_tool` method.
This appends the tool to the list of tools that the AI model will use.
...
- Explore the full [Language Model Request](/docs/concepts/generating-text) API.
